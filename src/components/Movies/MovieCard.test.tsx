...
import UserContextProvider, { UserContext, User } from '../../utils/UserContext';
...
describe('<MovieCard />', () => {
    test('renders correctly', () => {
        const wrapper = shallow(<MovieCard movie={shrek} onSelect={() => {}} onQuickReview={() => {}} />);
    ...
    test('onSelect is called on button press', () => {
        ...
        const wrapper = shallow(<MovieCard movie={shrek} onSelect={mockCallback} onQuickReview={() => {}} />);
    ...
    test('onQuickReview is called when quick review button is pressed', () => {
        ...
        const wrapper = mount(
            <UserContextProvider user={mockUser}>
                <MovieCard movie={shrek} onSelect={() => {}} onQuickReview={mockCallback} />
            </UserContextProvider>
        );
    ...
    test('quick review button is rendered if user is set in context', () => {
        ...
        expect(quickReviewButton.prop('title')).toContain(`Review ${shrek.Title} as ${mockUser.name}`);
    });
});
